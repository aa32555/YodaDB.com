...
describe("Main Navigation Module Tests", async () => {
  it("Navigate to the main navigation page", async () => {
    await page.goto(`http://localhost:${MDevPort}/YodaDB/`, {
      timeout: 0,
    ...
    expect(await page.url()).to.equal(
      `http://localhost:${MDevPort}/YodaDB/`
    );
  });
  ...
  it("Clicking on 'running processes module'", async () => {
    ...
    await delay(1500)
    expect(await page.url()).to.equal(
      `http://localhost:${MDevPort}/YodaDB/processes`
    );
  });
  ...
  it("Clicking on 'gde module'", async () => {
    ...
    await delay(1500)
    expect(await page.url()).to.equal(
      `http://localhost:${MDevPort}/YodaDB/gde`
    );
  });
  ...
  it("Clicking on 'routines explorer module'", async () => {
    ...
    await delay(1500)
    expect(await page.url()).to.equal(
      `http://localhost:${MDevPort}/YodaDB/routines`
    );
  });
  ...
  it("Clicking on 'globals explorer module'", async () => {
    ...
    await delay(1500)
    expect(await page.url()).to.equal(
      `http://localhost:${MDevPort}/YodaDB/globals`
    );
  });
  ...
  it("Clicking on 'octo explorer module'", async () => {
    ...
    await delay(1500)
    expect(await page.url()).to.equal(
      `http://localhost:${MDevPort}/YodaDB/octo-sql`
    );
  });
